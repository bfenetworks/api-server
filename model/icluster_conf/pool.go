...
func (p *Pool) GetDefaultInstances() *PoolInstances {
...
func (rppm *PoolManager) CreateBFEPool(ctx context.Context, pool *PoolParam, pis *PoolInstances) (one *Pool, err error) {
	pool.Tag = &PoolTagBFE
	return rppm.CreateProductPool(ctx, ibasic.BuildinProduct, pool, pis)
}

func (rppm *PoolManager) CreateProductPool(ctx context.Context, product *ibasic.Product, pool *PoolParam, pis *PoolInstances) (one *Pool, err error) {
	var pN string
